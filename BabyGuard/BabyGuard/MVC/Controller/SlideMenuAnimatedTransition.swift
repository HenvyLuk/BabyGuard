...
public class SlideMenuAnimatedTransition: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    public var revealAmount: CGFloat = UIScreen.mainScreen().bounds.width * 0.725
    ...
    override init() {
        ...
        menuContainerView.addGestureRecognizer(UIPanGestureRecognizer(target: self, action: #selector(SlideMenuAnimatedTransition.handlePanGesture(_:))))
        
        // Configure menu background view
        menuBackgroundView.backgroundColor = UIColor.blackColor().colorWithAlphaComponent(0.6)
        menuBackgroundView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(SlideMenuAnimatedTransition.handleTapGesture(_:))))
        menuBackgroundView.addGestureRecognizer(UIPanGestureRecognizer(target: self, action: #selector(SlideMenuAnimatedTransition.handlePanGesture(_:))))
    }
